...
> 命题依据：[设计-v0.5-配置工作台.md](../../internal/docs/设计-v0.5-配置工作台.md)（内部材料，不入公开仓库；API 契约唯一出处）、
> [待确认与决策.md](../../internal/docs/待确认与决策.md)（内部材料，不入公开仓库）D15（数据契约三层）、现有账套配置写法（retail 形态）、
