...
exe 从 GitHub Releases 页下载 `ClearLedger-Launcher.exe`（仓库 → Releases → 最新 launcher 版本），放在仓库根目录后双击（文件名随意）。图形引导窗自动完成同样的事，最后打开浏览器 `http://127.0.0.1:8620`。首次约 10–25 分钟（下载依赖），之后秒开。个别杀软可能对单文件 exe 误报，放行即可——源码就是本仓库，可自行审查。
